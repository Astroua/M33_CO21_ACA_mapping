...
for brick_num in {1..3}; do
    ...
    for tile_num in {1..5}; do
        ...
        (${casa_path} --nologger --nogui --log2term --nocrashreport --logfile ${data_path}/per_mosaic_imaging/logs/${logfile_name} -c ${code_path}/per_mosaic_cont_imaging.py ${brick_num} ${tile_num} ${rerun_existing}) &
        pids+=" $!"
