    ...
    for line in "${lines[@]}"; do
        for brick_num in {1..3}; do
            ...
            for tile_num in {1..5}; do
                ...
                (${casa_path} --nologger --nogui --log2term --nocrashreport --logfile ${data_path}/${logfile_name} -c ${code_path}/per_mosaic_line_imaging.py ${brick_num} ${tile_num} ${line} ${spec_width} ${rerun_existing}) &
                pids+=" $!"
